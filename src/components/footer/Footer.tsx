...
const Footer = () => {
  ...
  useEffect(() => {
    const fetchCompanySocialLinks = async () => {
      try {
        const res = await fetch("/api/cms/company-social-links");
        const data = await res.json();
        setCompanySocialLinks(data);
      } catch (err) {
        console.error("Hero fetch error:", err);
      }
    };

    fetchCompanySocialLinks();
  }, []);
      ...
      <div id="left" className="flex flex-col w-full order-2 md:order-1">
        ...
        >
          ...
          >
            ...
            <p className="text-xs sm:text-sm md:text-base">
              آدرس ایمیل :
              <a href={`mailto:${info?.email_address}`}>
                {" "}
                {info?.email_address}
              </a>
            </p>
            {info?.company_address != null && info?.company_address !== "" && (
              <p className="text-xs sm:text-sm md:text-base">
                آدرس شرکت : {info?.company_address}
              </p>
            )}
          </div>
        </div>
        ...
        {companySocialLinks && (
          <div className="flex gap-8 sm:gap-12 md:gap-16 justify-center sm:justify-start">
            {companySocialLinks[0] && (
              <Link href={companySocialLinks[0].url}>
            ...
            {companySocialLinks[1] && (
              <Link href={companySocialLinks[1].url}>
            ...
            {companySocialLinks[2] && (
              <Link href={companySocialLinks[2].url}>
