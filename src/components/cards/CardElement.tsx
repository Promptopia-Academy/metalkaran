...
export default function CardElement({ image, title, slug }: ICardElementProps) {
  return (
    <div
      ...
      className="group cursor-pointer bg-card w-full pb-3 md:pb-4 rounded-xl md:rounded-2xl shrink-0 overflow-hidden min-w-0 relative transition-all duration-500 ease-out hover:shadow-2xl hover:-translate-y-1 md:hover:-translate-y-2">
      ...
      <div className="flex flex-col gap-1.5 md:gap-2 justify-center items-center p-3 md:p-4">
        ...
        <Button
          variant={"default"}
          className="relative bg-primary mx-auto text-xs sm:text-sm md:text-base font-semibold px-4 sm:px-5 md:px-6 py-1.5 sm:py-2 transition-all duration-300 hover:scale-105 mt-1"
        >
          ...
          <span className="absolute inset-0 opacity-0 rounded-md transition-opacity duration-300" />
        </Button>
      </div>
