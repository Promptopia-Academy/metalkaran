import { IAboutUsPageDescription } from "@/types/type";
...
export async function getAboutUsDescriptions(): Promise<IAboutUsPageDescription[]> {
  ...
  if (!res.ok) {
    throw new Error("خطا در دریافت توضیحات صفحه درباره ما");
  }
...
export async function createAboutUsDescription(data: AboutUsDescriptionInput) {
  ...
  if (!res.ok) {
    throw new Error(json.error || "خطا در ایجاد توضیح");
  }
...
export async function updateAboutUsDescription(
  ...
  if (!res.ok) {
    throw new Error(json.error || "خطا در بروزرسانی توضیح");
  }
...
export async function deleteAboutUsDescription(id: number) {
  ...
  if (!res.ok) {
    throw new Error(json.error || "خطا در حذف توضیح");
  }
